...
export default class SyncComponentModule extends IModule {
    ...
    async OnCreate() {
        /**Zepeto Player Sync**/
        this.server.onMessage(MESSAGE.SyncPlayer, (client: SandboxPlayer, message: any) => {
        ...
        /**Transform Sync**/
        this.server.onMessage(MESSAGE.SyncTransform, (client: SandboxPlayer, message: any) => {
        ...
        this.server.onMessage(MESSAGE.SyncTransformStatus, (client: SandboxPlayer, message: any) => {
        ...
        /** Sync Animaotr **/
        this.server.onMessage(MESSAGE.SyncAnimator, (client: SandboxPlayer, message: any) => {
        ...
        /** SyncTransform Util **/
        this.server.onMessage(MESSAGE.ChangeOwner, (client: SandboxPlayer, message: string) => {
            this.server.broadcast(MESSAGE.ChangeOwner + message, client.sessionId);
        });
        this.server.onMessage(MESSAGE.Instantiate, (client: SandboxPlayer, message: InstantiateObj) => {
        ...
        this.server.onMessage(MESSAGE.RequestInstantiateCache, (client: SandboxPlayer) => {
        ...
        /**SyncDOTween**/
        this.server.onMessage(MESSAGE.SyncDOTween, (client: SandboxPlayer, message: syncTween) => {
        ...
        /**Common**/
        this.server.onMessage(MESSAGE.CheckServerTimeRequest, (client: SandboxPlayer, message:string) => {
            let Timestamp = +new Date();
            client.send(MESSAGE.CheckServerTimeResponse, Timestamp);
        });
        this.server.onMessage(MESSAGE.CheckMaster, (client: SandboxPlayer, message:string) => {
            console.log(`master->, ${this.sessionIdQueue[0]}`);
            this.server.broadcast(MESSAGE.MasterResponse, this.sessionIdQueue[0]);
        });
        this.server.onMessage(MESSAGE.PauseUser, (client: SandboxPlayer) => {
        ...
        this.server.onMessage(MESSAGE.UnPauseUser, (client:SandboxPlayer) => {
            if (!this.sessionIdQueue.includes(client.sessionId)) {
        ...
        /** Sample Code **/
        this.server.onMessage(MESSAGE.BlockEnter, (client:SandboxPlayer, transformId: string) => {
            this.server.broadcast(MESSAGE.BlockEnter + transformId, client.sessionId);
        });
        this.server.onMessage(MESSAGE.BlockExit, (client:SandboxPlayer, transformId: string) => {
            this.server.broadcast(MESSAGE.BlockExit + transformId, client.sessionId);
        });
        this.server.onMessage(MESSAGE.SendBlockEnterCache, (client:SandboxPlayer, blockCache : any) => {
            this.server.loadPlayer(blockCache.newJoinSessionId)?.send(MESSAGE.BlockEnter + blockCache.transformId, client.sessionId);
        });

        this.server.onMessage(MESSAGE.CoinAcquired, (client:SandboxPlayer, transformId: string) => {
        ...
        this.server.onMessage(MESSAGE.StartRunningRequest, (client:SandboxPlayer) => {
            if (!isStartGame) {
                isStartGame = true;
        ...
        this.server.onMessage(MESSAGE.FinishPlayer, (client:SandboxPlayer, finishTime: number) => {
